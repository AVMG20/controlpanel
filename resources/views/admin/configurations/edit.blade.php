...
@section('head')
    ...
    <style>
        .ts-control .item {
            background-color: rgba(var(--bs-gray-800-rgb), 1) !important;
        ...
        <div class="card card-body border-0 shadow table-wrapper table-responsive">
            <form method="post"
                ...
                <div class="d-flex justify-content-between">
                    <h2 class="mb-4 h5">{{ isset($configuration) ? __('Edit configuration') : __('Create configuration') }}
                    </h2>
                    <div class="form-check form-switch ">

                        <input class="form-check-input" name="disabled" value="1"
                            @if (isset($configuration) && $configuration->disabled) checked @endif type="checkbox" id="flexSwitchCheckDefault">
                ...
                <div class="row">
                    <div class="col-lg-6">
                        ...
                        <x-input.text label="{{ __('Name') }}" name="name"
                            value="{{ isset($configuration) ? $configuration->name : null }}" />

                        <x-input.textarea label="{{ __('Small description') }}" name="description" ckeditor
                        ...
                        <div class="row mt-4">
                            <div class="col-lg-6">
                                ...
                                <x-input.number label="{{ __('Memory') }}" name="memory" min="0" max="999999999"
                                    prepend="{{ __('MB') }}"
                                ...
                                <x-input.number label="{{ __('Cpu') }}" name="cpu" min="0" max="999999999"
                                    prepend="%"
                                ...
                                <x-input.number label="{{ __('Disk space') }}" name="disk" min="0"
                                    max="999999999" prepend="{{ __('MB') }}"
                                ...
                                <x-input.number label="{{ __('Swap') }}" name="swap" min="0" max="999999999"
                                    prepend="{{ __('MB') }}"
                                ...
                                <x-input.number label="{{ __('Block IO Weight') }}" name="io" min="0"
                                    max="999999999"
                            ...
                            <div class="col-lg-6">
                                ...
                                <x-input.number label="{{ __('Database Limit') }}" name="databases" min="0"
                                    max="999999999"
                                ...
                                <x-input.number label="{{ __('Backup Limit') }}" name="backups" min="0"
                                    max="999999999"
                                ...
                                <x-input.number label="{{ __('Allocation Limit') }}" name="allocations" min="0"
                                    max="999999999"
                    ...
                    <div class="col-lg-6">
                        ...
                        <x-input.number label="{{ __('Price per month') }}" name="price" min="0"
                            max="9999999999999" step=".000001"
                            tooltip="{{ __('Price per month. Servers are charged hourly (price / 30 / 24)') }}"
                            value="{{ isset($configuration) ? $configuration->price : null }}" />

                        <x-input.number label="{{ __('Setup price') }}" name="setup_price" min="0"
                            max="9999999999999" step=".000001"
                        ...
                        <x-input.number
                            ...
                            name="minimum_required_credits" min="0" max="9999999999999" step=".000001"
                        ...
                        <x-input.select label="{{ __('Locations') }}" multiple
                            tooltip="{{ __('Number within () is the ID of the given resource') }}" name="locations"
                            class="smart-select">
                            @foreach ($locations as $location)
                                <option @if (isset($configuration) && $configuration->locations->contains($location)) selected @endif value="{{ $location->id }}">
                                    ({{ $location->id }})
                                    {{ $location->name }}</option>
                            @endforeach
                        </x-input.select>

                        <x-input.select label="{{ __('Eggs') }}" multiple
                            tooltip="{{ __('Number within () is the ID of the given resource') }}" style="height: 180px;"
                            name="eggs" class="smart-select">
                            @foreach ($eggs as $egg)
                                <option @if (isset($configuration) && $configuration->eggs->contains($egg)) selected @endif value="{{ $egg->id }}">
                                    ({{ $egg->id }})
                                    {{ $egg->name }}</option>
                            @endforeach
                        </x-input.select>

                        {{ __(' No Eggs / Locations showing?') }} <a href="{{ route('admin.configurations.sync') }}"><i
                                style="color:blue;">{{ __('Sync now') }}</i></a>
